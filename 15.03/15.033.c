...
// HL7 formatýna uygun hasta bilgisi oluþturma
void hastaToHL7(Hasta *hasta) {
    char hl7Message[1024];

    // HL7 PID Segmenti
    snprintf(hl7Message, sizeof(hl7Message),
             "PID|1||%s||%s^%s||%d|%s|||%s|||%s||%d",
             hasta->tc, hasta->isim, hasta->soyisim, hasta->yas, hasta->cinsiyet,
             hasta->sikayet, hasta->oncelik == 1 ? "Cok Acil" : (hasta->oncelik == 2 ? "Acil" : "Normal"), hasta->receteNo);
...
void hastaEkle() {
    ...
    if (yeniHasta.yas < 18) { // Cocuk hastalar icin
        ...
        // HL7 formatýnda hasta bilgisini yazdýr
        hastaToHL7(&yeniHasta);
